...
help: ## Mostra esta ajuda
	...
	@echo ""
	@grep -E '^[a-zA-Z_-]+:.*?## .*$$
' $(MAKEFILE_LIST) | \ awk 'BEGIN {FS = ":.*?## "}; {printf "  $(GREEN)%-15s$(RESET) %s\n",
$$1, $$2}'
	@echo ""
...
install-dev: ## Instala dependências de produção + dev
	pip install -r requirements.txt
...
test-cov: ## Roda testes e abre relatório HTML
	pytest
	@echo "$(CYAN)→ Relatório: htmlcov/index.html$(RESET)"

test-smoke: ## Roda apenas testes smoke (críticos)
	pytest -m smoke
...
audit: ## Auditoria de vulnerabilidades em deps
	pip-audit -r requirements.txt
	pip-audit -r requirements-dev.txt

# ===== Pipeline Completo =====
check: lint security test ## Roda lint + security + tests (CI local)
