...
CSAT implementation in zendesk_service.py:
  - get_satisfaction_ratings() filters raw API scores to ("good", "bad") only
    at line 141: [r for r in raw if r.get("score") in ("good", "bad")]
...
Edge contract: total=0 (no answered ratings) → csat_pct=0.0 (pessimistic,
per implementation at line 155: `if total else 0.0`).
"""
...
class TestCSATExclusion:
    ...
    def test_unoffered_excluded_from_denominator(self, svc: ZendeskService) -> None:
        """3 good + 1 bad + 10 unoffered → 75%, not ~21%."""
        ratings = (
        ...
        respx.get(RATINGS_URL).mock(return_value=httpx.Response(200, json=_ratings_page(ratings)))
        summary = svc.get_csat_summary()
        # Only good+bad count — unoffered stripped at ingest (service.py:141)
        assert summary.total_ratings == 4
        assert summary.csat_pct == 75.0

    @respx.mock
    def test_offered_no_response_excluded(self, svc: ZendeskService) -> None:
        """3 good + 1 bad + 5 offered → 75%, not ~43%."""
        ratings = (
            [_rating("good", i) for i in range(1, 4)]
            + [_rating("bad", 4)]
    ...
    def test_mixed_unoffered_offered_and_answered(self, svc: ZendeskService) -> None:
        """Mix of all states: only good/bad count in total_ratings and csat_pct."""
        ratings = [
            _rating("good", 1),
            _rating("good", 2),
            _rating("good", 3),
...
class TestCSATEdgeCases:
    @respx.mock
    def test_only_unoffered_returns_zero(self, svc: ZendeskService) -> None:
        """Only unoffered — no answered ratings → csat_pct=0.0 (implementation contract)."""
        ratings = [_rating("unoffered", i) for i in range(1, 6)]
        respx.get(RATINGS_URL).mock(return_value=httpx.Response(200, json=_ratings_page(ratings)))
        summary = svc.get_csat_summary()
        assert summary.total_ratings == 0
        assert summary.csat_pct == 0.0  # `if total else 0.0` — pessimistic default

    @respx.mock
    def test_empty_ratings_returns_zero(self, svc: ZendeskService) -> None:
        """Empty list → total=0, csat_pct=0.0, no error."""
